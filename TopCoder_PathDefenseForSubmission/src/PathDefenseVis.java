...
public class PathDefenseVis {
    ...
    public int runTest() {
        ...
        for (int t = 0; t < Constants.SIMULATION_TIME; t++) {
            ...
            int commandCnt;
            try {
            	System.out.println("startReading from my program of time: " + t);
                commandCnt = Integer.parseInt(reader.readLine());
//                System.out.println("commandCnt: " + commandCnt);
                ...
                if (commandCnt>0) {
                    ...
                    for (int i=0;i<newTowers.length;i+=3) {
                        ...
                        if (tc.board[newT.y][newT.x]!='#') {
                            System.err.println("ERROR: time step = " + t + " ERROR: Cannot place a tower at (" + newT.x + "," + newT.y + ").");
                            return -1;
                        }
